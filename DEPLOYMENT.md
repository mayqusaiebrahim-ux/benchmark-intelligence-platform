...
A Render Blueprint pre-wired with the correct build command (`npm install` — this alone triggers the full `postinstall` chain in §4.1, including both Playwright browser installs; no separate `--with-deps` or top-level `npx playwright install` call — see §1's Playwright row for why), `PLAYWRIGHT_BROWSERS_PATH=0`, start command, a persistent disk mount, and the `OPENAI_API_KEY` secret slot. Includes an inline comment flagging the §3.3 port issue so it isn't missed at deploy time.
